      ...
      <div class="row">
            ...
            @csrf
                <div class="row">
                    ...
                    @if($forms['type'] == "text" || $forms['type'] == "hidden" || $forms['type'] == "number")
                        <div class="input-field col {{ $forms['col'] }}">
                            <input placeholder="{{ $forms['placeholder'] }}" name="{{ $forms['name'] }}" id="{{ $forms['name'] }}" type="{{ $forms['type'] }}" class="validate  @error('nama_fakultas') is-invalid @enderror" value="{{ old($forms['name'],$forms['data']) }}">
                            @if($forms['type'] != 'hidden')<label for="first_name">{{ $forms['placeholder'] }}</label>@endif
                  ...
                  <a href="{{ route('typemahasiswa.index') }}" class="waves-effect purple darken-1 btn-small"><i class="material-icons left">keyboard_arrow_left</i>back</a>
                  </div>
                </div>
            </form>
        </div>
