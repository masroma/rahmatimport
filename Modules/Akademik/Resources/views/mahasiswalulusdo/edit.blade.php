                        ...
                        <div class="col s12">
                            <div class="card">
                                <div class="card-content">
                                    ...
                                    <div class="row">
                                        <form action="{{ route('mahasiswalulusdo.update', [$mahasiswalulusdo->id]) }}"
                                                ...
                                                @elseif($forms['type'] === 'date')
                                                    ...
                                                    <div class="input-field col {{ $forms['col'] }}">
                                                        <textarea placeholder="{{ $forms['placeholder'] }}" name="{{ $forms['name'] }}" id="{{ $forms['name'] }}"
                                                            class="validate materialize-textarea @error('nama_fakultas') is-invalid @enderror">{{ $forms['data'] }}</textarea>
                                                ...
                                                <div class="input-field col {{ $forms['col'] }}">
                                                    <select name="{{ $forms['name'] }}" id="{{ $forms['name'] }}">
                                                        ...
                                                        @foreach ($forms['relasi'] as $p)
                                                            <option value="{{ $p[$forms['id']] }}" {{$p['id']==$forms['data']?'selected':''}}>
                                                                {{$p['nim']??''}} 
                                                                {{ $p[$forms['select_name']] }} {{ $p->Tahunajaran->tahun_ajaran??'' }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                    ...
                                                    @elseif($forms['type']==='transkrip')
                                                <div class="input-field col s6">
                                                    <div class="row">
                                                        <div class="col s3">
                                                            <input type="text" class="validate" readonly value="0" name="" id="ipk">
                                                        </div>
                                                        <button type="button" class="row waves-effect waves-light btn-small col s9 right" onclick="getIpk()" data-id="{{$forms['data']}}">[ Tarik IPK hasil perhitungan data transkrip mahasiswa ]</button>
                                                    </div>
                                                </div>
                                                @endif
                                            @endforeach
...
@section('script')
    <script>
        function getIpk(){
            $.get('/akademik/mahasiswalulusdo/getipk/'+$('select[name="mahasiswa_id"]').val(),function(data){
                $('#ipk').val(data)
            })
        }
