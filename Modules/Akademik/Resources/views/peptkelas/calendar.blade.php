                    ...
                    <div class="row">
                      <div class="col m3 s12">
                        <div id='external-events'>
                          ...
                          <h6>Daftar BATCH PEPT</h6>

                          <div class="fc-events-container mb-5" style="
                          height: 450px;
                          overflow: scroll;">

                                @foreach($getpept as $dk)
                                <div class='fc-event' id="event" idpept="{{ $dk->id }}" data-color='{{ $dk->color }}'>{{ $dk->nama_batch }}{{ $dk->kode }}</div>
                                @endforeach
  ...
  var calendar = new Calendar(calendarEl, {
    ...
    drop: function (info, date) {
      ...
      dayName = days[d.getDay()];
       ...
       var idpept = info.draggedEl.attributes.idpept.nodeValue;
       var penggunaan = 6;
       var ruang = idruangan;
       ...
       var hari = dayName
        ...
        $.ajax({
            url:"{{ route('peptkelas.savecalendar') }}",
            data: {
                idpept: idpept,
    ...
    eventDrop:function(info){
        ...
        $.ajax({
            url:"{{ route('peptkelas.updatecalendar') }}",
            data: {
                idkelas: idkelas,
                penggunaanruang: penggunaan,
                jamawal: starttime,
