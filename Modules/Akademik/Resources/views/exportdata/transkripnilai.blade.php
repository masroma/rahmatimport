    ...
    <table style="border-bottom-style:double;padding-bottom:20px;margin-bottom:10px;" width="100%">
        <tbody>
            ...
            <tr>
                ...
                <td align="left"><strong>:</strong> {{ $selTN[0]->tahun_ajaran.' - '.$selTN[0]->jenis_semester }}</td>
            </tr>
            <tr>
                <td align="left"><strong>Program Studi</strong></td>
                <td align="left"><strong>:</strong> {{ $selTN[0]->nama_jenjang.' '.$selTN[0]->nama_jurusan }}</td>
    ...
    <table width="100%" border="1">
        <tbody>
            <tr>
                ...
                <th rowspan="2">Semester<br>Diambil</th>
                <th rowspan="2">Bobot MK<br>(sks)</th>
                <th colspan="3">Nilai</th>
                <th rowspan="2">Asal Nilai</th>
            </tr>
            <tr>
                <th>Angka</th>
                <th>Huruf</th>
            ...
            @php $no = 1; @endphp
            @foreach($selTN as $stn)
            <tr>
                <td align="center">{{ $no++ }}</td>
                <td align="center">{{ $stn->nim }}</td>
                <td>{{ $stn->nama }}</td>
                <td>{{ $stn->nama_jenjang.' '.$stn->nama_jurusan }}</td>
                <td align="center">{{ $stn->kode_matakuliah }}</td>
                <td>{{ $stn->nama_matakuliah }}</td>
                <td align="center">{{  number_format((float)$stn->bobot_mata_kuliah, 2, '.', '')  }}</td>
                <td align="center">{{  number_format((float)$stn->bobot_mata_kuliah, 2, '.', '')  }}</td>
                <td align="center">{{  number_format((float)$stn->nilai_angka, 1, '.', '')  }}</td>
                <td align="center">{{ $stn->nilai_huruf }}</td>
                <td align="center">{{  number_format((float)$stn->nilai_index, 2, '.', '')  }}</td>
                <td align="center">Kelas Kuliah</td>
            </tr>
            @endforeach
        </tbody>
    </table>
