            ...
            @csrf
                <div class="row">
                    @foreach ($form as $forms)
                        ...
                        @if(($forms['type'] == "text") || ($forms['type'] == "number"))
                        <div class="input-field col {{ $forms['col'] }}">
                            <input placeholder="{{ $forms['placeholder'] }}" name="{{ $forms['name'] }}" id="{{ $forms['name'] }}" type="{{  $forms['type'] }}" class="validate " value="{{ old($forms['name']) }}" step=".01">
