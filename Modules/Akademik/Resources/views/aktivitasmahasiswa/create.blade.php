            ...
            @csrf
                <div class="row">
                  ...
                  @elseif($forms['type'] === "date")
                      <div class="input-field col {{ $forms['col'] }}">
                      <input placeholder="{{ $forms['placeholder'] }}" name="{{ $forms['name'] }}" id="{{ $forms['name'] }}" type="date" step="any" class="validate  @error('nama_fakultas') is-invalid @enderror" value="{{ old($forms['name']) }}">
                            <label for="first_name">{{ $forms['placeholder'] }}</label>
                            <?php $error = $forms['name'];?>
                            @error($error)
                            <span class="red-text text-darken-2">{{ $message }}</small>
                      ...
                      <div class="input-field col {{ $forms['col'] }}">
                        ...
                        <option value="">Pilih</option>
                            @foreach($programstudy as $row)
                                <option value="{{$row->id}}">{{ $row->jurusan->nama_jurusan }}</option>
                            @endforeach
                        </select>
